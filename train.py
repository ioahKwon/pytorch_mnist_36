...
ckpt_dir = './checkpoint' # 학습된 네트워크 저장
log_dir = './log' # tensorboard 파일 저장
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.convl = nn.Conv2d(in_channels=1, out_channels=10, kernel_size=5, stride=1, padding=0, bias=True)
...
def save(ckpt_dir, net, optim, epoch):
    if not os.path.exisits(ckpt_dir):
        os.makedir(ckpt_dir)
...
dataset = datasets.MNIST(download=True, root ='./', train=True, transforms=transform)
loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=0)

num_data = len(loader.dataset)
num_batch = np.ceil(num_data / batch_size)

## 네트워크 설정 및 필요한 loss function 구현하기
net = Net().to(device)
